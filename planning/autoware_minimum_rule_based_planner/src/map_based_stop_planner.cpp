...
MapBasedStopPlanner::Result MapBasedStopPlanner::plan(
  const Trajectory & trajectory, const double ego_arc_length,
...
std::optional<MapBasedStopPlanner::SingleStopResult> MapBasedStopPlanner::plan_single_stop(
  ...
  const StopSelectionParams & params, const bool include_possibility,
  const std::optional<double> & road_shoulder_stop_arc_length) const
{
  ...
  auto stop_point = select_stop_point(
  ...
  if (
    ...
    (!stop_point || *road_shoulder_stop_arc_length < stop_point->arc_length)) {
    stop_point = StopPoint{*road_shoulder_stop_arc_length, StopLineType::RoadShoulder};
  }
